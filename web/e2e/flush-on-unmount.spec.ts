...
test("typing and immediately switching pages persists the edit without waiting for Saved", async ({ page }) => {
  ...
  await page.locator("aside").getByText("Flush Test B", { exact: true }).click();
...
test("leaving during a retry backoff still saves the edit via the unmount flush", async ({ page }) => {
  ...
  await expect(page.getByText("Failed to save", { exact: false })).toBeVisible({ timeout: 8000 });
  await page.locator("aside").getByText("Flush Retry Test B", { exact: true }).click();
