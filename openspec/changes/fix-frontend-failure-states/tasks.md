...
- [x] 6.1 Update `web/AGENTS.md` (or `web/CLAUDE.md`, whichever documents the API client) to record that the 401 redirect is opt-in and why, so the next write endpoint does not rediscover this — and verify the note names `saveDoc` as the case that motivated it — both are `web/.gitignore`d (`web/AGENTS.md` is regenerated by `next dev`; `web/CLAUDE.md` is a local override), so neither can hold committed documentation; the note lives instead as `apiFetch`'s doc comment in `web/lib/api.ts` (added in group 3), which already names `saveDoc` as the motivating case for every future write endpoint reading that function
- [x] 6.2 Check whether the README or e2e documentation describes the test suite's capabilities, and add the failure-injection helpers if so, recording a no-op check here if not — no-op: neither the root nor `web/` README describes test-suite capabilities beyond the run command, and no `e2e/README` exists; there's no capabilities doc to extend
