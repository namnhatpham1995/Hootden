...
- [ ] 1.1 Add `async rewrites()` to `web/next.config.ts` forwarding `/api/:path*` to `${API_PROXY_TARGET}/:path*`, defaulting `API_PROXY_TARGET` to `http://localhost:8080` for local dev, and verify `next build` produces a routes manifest containing the rule
- [ ] 1.2 Change `web/lib/api.ts` to call relative `/api/...` paths instead of building an absolute URL from `API_ORIGIN`/`NEXT_PUBLIC_API_ORIGIN`, and update its doc comment that currently cites the now-reversed "never through a Next.js route handler" decision
- [ ] 1.3 Rename `NEXT_PUBLIC_API_ORIGIN` to `API_PROXY_TARGET` in `web/Dockerfile` (still a build `ARG`, now feeding the rewrite manifest instead of the client bundle), `web/.env.example`, and `docker-compose.yml`'s `web` service build args
