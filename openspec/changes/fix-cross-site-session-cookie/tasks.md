...
- [x] 1.4 Set `API_PROXY_TARGET=<railway-origin>` on Vercel and trigger a redeploy — it is baked into the routes manifest at build time, so a variable change alone does nothing — then verify the deployed app's `/api/*` requests reach Railway, and check whether a Vercel preview deployment's `/api/*` calls now also reach it (expected side effect of the proxy; note the actual result for task 5.3). `API_PROXY_TARGET` was already set to `https://hootden-production.up.railway.app` (production + preview) and production's current deployment (commit `0867b43e`, PR #48) already built with it — no redeploy was needed. Verified live: `curl https://hootden.vercel.app/api/healthz` returns `{"status":"ok"}` with `X-Railway-Edge`/`X-Railway-Request-Id` headers, confirming the proxy reaches Railway. `API_PROXY_TARGET`'s target includes `preview`, so a preview deployment should also proxy `/api/*` to Railway by the same mechanism — not empirically verified against a live preview build, since that would mean deploying one; task 5.3 should confirm on an actual preview deployment before deciding whether to drop or keep the README's preview caveat.
- [x] 1.5 Set `API_ORIGIN` on Railway equal to `APP_ORIGIN`'s value (the Vercel origin) and leave `COOKIE_DOMAIN` empty, and verify the server still starts — the existing `checkCookieReachability` check from group 2 passes because the hosts are now equal. Set `API_ORIGIN=https://hootden.vercel.app` on the Railway `Hootden` service; `COOKIE_DOMAIN` was already unset. Triggered redeploy succeeded (deployment `6f8e0555`, status `SUCCESS`) and the deploy log shows `listening on :8080` with no startup error, confirming `checkCookieReachability` passed.
